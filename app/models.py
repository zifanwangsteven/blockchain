...
class AlgorandAccount(db.Model):
    __tablename__ = 'algorand_accounts'
    id = db.Column(db.Integer, primary_key = True)
    public_address = db.Column(db.String(64), unique=True)
    account_val = db.Column(db.Float)
    active_since = db.Column(db.DateTime(), default=datetime.utcnow)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    def refresh(self, algod_client):
        self.account_val = algod_client.account_info(self.public_address).get('amount')
